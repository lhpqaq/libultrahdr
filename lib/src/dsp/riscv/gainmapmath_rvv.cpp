...
void convert_rgb_to_yuv_rvv(uhdr_raw_image_ext_t* dst, const uhdr_raw_image_t* src) {
    ...
    for (size_t i = 0; i < dst->h; i++) {
        for (size_t j = 0; j < dst->w; j += vl) {
            ...
            vfloat32m8_t vg = __riscv_vreinterpret_v_u32m8_f32m8(__riscv_vand_vx_u32m8(__riscv_vsrl_vx_u32m8(vrgb, 8, vl), 0xff, vl));
            vfloat32m8_t vb = __riscv_vreinterpret_v_u32m8_f32m8(__riscv_vand_vx_u32m8(__riscv_vsrl_vx_u32m8(vrgb, 16, vl), 0xff, vl));
